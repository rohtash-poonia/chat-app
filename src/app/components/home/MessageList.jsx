...
const MessageList = ({ setChatPerson }) => {
  ...
  return (
    <div className="max-w-87.25 w-full flex h-screen flex-col items-center shadow-[1px_0px_0px_0px_rgba(0,0,0,0.08)] ">
      <div className=" w-full  border-b-2 border-[#F3F3F3] flex justify-between items-center py-7 px-6">
      ...
      {/* for search */}
      <div className="max-w-87.25 w-full py-3 flex justify-center items-center">
        <input
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          type="search"
          placeholder="search  Search messages"
          className="max-w-75.25 h-12 outline-none rounded-xl px-5 w-full bg-[#F3F3F3]"
        />
      </div>
      {/* all chat members */}
